...
async def read_file_content(file: UploadFile) -> pd.DataFrame:
    """Read uploaded file content into pandas DataFrame."""
...
def insert_data_in_batches(conn, df: pd.DataFrame, table_name: str, batch_percentage: int) -> tuple:
    """Insert data using psycopg2 execute_values in percentage-based batches."""
    df = df.astype(str)  # Convert all data to strings
    total_rows = len(df)
    batch_size = int(total_rows * (batch_percentage / 100))
...
async def process_upload_to_postgres(file: UploadFile, db_params: DatabaseParams) -> tuple:
    """Process uploaded file and insert data into PostgreSQL."""
    ...
    try:
        ...
        total_rows, batch_size, num_batches = insert_data_in_batches(
            conn, df, db_params.table_name, db_params.batch_percentage
        )
        return total_rows, batch_size, num_batches
    except Exception as e:
        logger.error(f"Error in processing file {file.filename}: {str(e)}")
        raise
    finally:
        conn.close()
...
async def upload_file(
    ...
    db_params: DatabaseParams = Depends(),
    username: str = Depends(authenticate)
):
    """
    Upload a file (CSV or Excel) and process it into the database.
    """
    try:
        ...
        total_rows, batch_size, num_batches = await process_upload_to_postgres(file, db_params)
